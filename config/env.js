...
// Grab NODE_ENV and SHINOBI_* environment variables and prepare them to be
// injected into the application via DefinePlugin in Webpack configuration.
const SHINOBI = /^SHINOBI_/i;

function getClientEnvironment(target, options) {
  const raw = Object.keys(process.env)
    .filter(key => SHINOBI.test(key))
    .reduce(
      (env, key) => {
      ...
      },
      {
        ...
        SHINOBI_ASSETS_MANIFEST: paths.appManifest,
        WEBPACK_ASSETS_MANIFEST: paths.appManifest,
        ...
        // It's useful if you're running shinobi in a non-localhost container. Ends in a /
